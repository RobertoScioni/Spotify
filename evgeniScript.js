window.onload = () => {
  ...
  fetch(`https://deezerdevs-deezer.p.rapidapi.com/album/${urlID}`, {
    ...
    .then((data) => {
      ...
      let getTracks = document.querySelector(".albumLength > span");
      getArtistName.setAttribute(
        "onclick",
        `window.location="artist.html?id=${data.artist.id}"`
      );
      getTracks.innerText = `${data.release_date} • ${data.tracks.data.length} SONGS`;
